...
Auth strategy:
  - App runs as a service principal (M2M OAuth via injected CLIENT_ID/SECRET).
  - The SP fetches a user PAT from Databricks Secrets (scope: retail-insights).
  - All Lakebase calls are made using that PAT so credentials are generated for
    p.amani@gmail.com — the Lakebase project owner — rather than the SP.
  - SDK client and endpoint metadata cached permanently (_init_sdk).
  - Lakebase token generated fresh per query (1 h TTL).
...
def _init_sdk():
    ...
    # SP fetches the owner's PAT from Databricks Secrets, then impersonates the
    # owner for Lakebase (the SP itself has no Lakebase platform-level access).
    sp_w = WorkspaceClient()
    raw_b64 = sp_w.secrets.get_secret(scope="retail-insights", key="lakebase-token").value
    user_pat = base64.b64decode(raw_b64).decode()

    host = os.environ.get("DATABRICKS_HOST", "")
    # SDK errors if both PAT and M2M env vars are present simultaneously.
    # Pop them for the duration of this one constructor call.
...
cust_kpi = query("""
    SELECT COUNT(*) AS customers, ROUND(AVG(monetary)::numeric, 2) AS avg_ltv
...
prod_kpi = query(f"""
    SELECT
        ...
        ROUND(AVG(avg_discount_pct)::numeric, 1)       AS avg_discount
...
with t_products:
    ...
    prods = query(f"""
        SELECT product_name, category, brand,
               ...
               ROUND((gross_profit / NULLIF(gross_revenue, 0) * 100)::numeric, 1) AS margin_pct
        FROM gold_product_performance
        WHERE category IN ({cats_in})
